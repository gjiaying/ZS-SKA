...
class Category:
    ...
    def __init__(self, label, description, hierarchy, hypernym1, hypernym2): # Create an empty path
        self.label = label.strip().lower()
        self.description = description.strip().lower()
        self.hierarchy = hierarchy.strip().lower().split(';')
        self.hypernym1 = hypernym1.strip().lower()
        self.hypernym2 = hypernym2.strip().lower()
    ...
    def find_nodes(self):
        ...
        # Hypernym1 class
        self.nodes['hypernym1'] = get_all_nodes_from_label(self.hypernym1)
        
        # Hypernym2 class
        self.nodes['hypernym2'] = get_all_nodes_from_label(self.hypernym2)
...
def load_ConceptNet():
    ...
    filename = 'conceptnet-assertions-5.6.0.csv'
...
load_ConceptNet()
class_nodes = set()
class_info = get_class_info(os.getcwd()+ '/labels/' + 'NYT_labels.csv')
classes = [Category(c['ClassLabel'], c['Hierarchy'], c['ClassDescription'], c['Hypernym1'], c['Hypernym2']) for c in class_info]
for c in classes:
...
for c in tqdm(class_nodes):
    ...
    NODES_DATA[c].find_neighbors(1)
pickle.dump(NODES_DATA, open('NODES_DATA.pickle', "wb"))

for c in tqdm(classes):
    all_c_nodes = c.get_all_nodes()
    all_neighbors = get_neighbors_of_cluster(all_c_nodes, hop = 1)
    print(c, len(all_neighbors))
    vectors = dict()
    for n in all_neighbors:
        vectors[n] = np.concatenate((get_vector_of(n, c.nodes['the_class'], hop = 1), get_vector_of(n, c.nodes['super_class'], hop = 1), get_vector_of(n, c.nodes['description'], hop = 1), get_vector_of(n, c.nodes['hypernym1'], hop = 1), get_vector_of(n, c.nodes['hypernym2'], hop = 1)), axis = 0)
    #print(vectors)
    #print(len(vectors))
    pickle.dump(vectors, open(os.getcwd()+ '/KG_VECTOR_NYT1/' + 'VECTORS_CLUSTER_1_' + c.label + ".pickle", "wb"))
