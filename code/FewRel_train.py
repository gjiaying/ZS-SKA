...
with (open(os.getcwd()+"/data/Graph_0.5_10.pickle", "rb")) as openfile:
...
labels = []
EPOCH = 500
TEST_FILE = 'FewRel_test_m15unseen.txt'
TRAIN_FILE = 'FewRel_all_train.txt'
LABEL = 'FewRel_labels.csv'
...
def get_feature_embedding(file):
    ...
    while 1:
        ...
        if wn.synsets(line1['h']['name']) == []:
            head = "None"
        else:
            heads = wn.synsets(line1['h']['name'])[0]
            for i in range(len(heads.hypernyms())):
                head = heads.hypernyms()[i].name().split('.')[0]
        head_embedding = oovcheck_t((head).lower())
        
        if wn.synsets(line1['t']['name']) == []:
            tail = "None"
        else: 
            tails = wn.synsets(line1['t']['name'])[0]
...
def get_test_embedding(model_u, loader, featest_embedding):
    out_embedding = []
    ys = []
    for x, y in loader:
        ys.append(y.cuda())
...
for e in range(EPOCH):
    ...
    for x, y in train_loader:
        ...
        for i in range(len(y)):
            prompt_embedding = get_prompt_embedding(int(y[i].item()))
            y_list.append(prompt_embedding)
            
            new =  np.concatenate((y_pred[i].cpu().detach().numpy(),prompt_embedding),axis=None)
            output.append(new)
            labels.append(y[i].item())
               
        prompt = torch.Tensor(y_list).cuda()
        result = torch.cat((y_pred,prompt),1)
...
featest_embedding = get_feature_embedding(os.getcwd()+ '/data/' + TEST_FILE)

truth_labels, test_embedding = get_test_embedding(model_encoder, test_loader, featest_embedding)
